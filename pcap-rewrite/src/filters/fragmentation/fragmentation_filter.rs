...
pub struct FragmentationFilter<C, D> {
    data_hs: HashSet<TwoTupleProtoIpidFiveTuple>,
    convert_data_hs_c: Box<dyn Fn(&HashSet<TwoTupleProtoIpidFiveTuple>) -> C>,
    key_container: C,
...
impl<C, D> FragmentationFilter<C, D> {
    pub fn new(
        data_hs: HashSet<TwoTupleProtoIpidFiveTuple>,
        convert_data_hs_c: Box<dyn Fn(&HashSet<TwoTupleProtoIpidFiveTuple>) -> C>,
        key_container: C,
    ...
    fn test_fragmentation_and_save(&mut self, packet: &Packet) -> Result<(), String> {
        ...
        if is_first_fragment {
            ...
            match data_option {
                None => panic!("could find ip_fragment but not 3/5 tuple?"),
...
impl<C, D> Filter for FragmentationFilter<C, D> {
...
pub fn test_two_tuple_proto_ipid_five_tuple_option_in_container(
    c: &(TwoTupleProtoIpidC, FiveTupleC),
        ...
        match filtering_key {
            ...
            FilteringKey::SrcDstIpaddrProtoSrcDstPort => {
                ...
                let keep: &dyn Fn(
                    ...
                    FilteringAction::Keep => &|c, three_five_tuple| {
                        test_two_tuple_proto_ipid_five_tuple_option_in_container(
                            c,
                            three_five_tuple,
                        )
                    },
                    FilteringAction::Drop => &|c, three_five_tuple| {
